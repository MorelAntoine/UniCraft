using UniCraft.Character.System;
using UnityEngine;

namespace UniCraft.Character.MotionStateMachine.State.Locomotion
{
	[CreateAssetMenu(menuName = "UniCraft/Character/State/Locomotion/Walk")]
	public class WalkState : AMotionState
	{			
		protected override void Run2D(ACharacterSystem2D cs2D, MotionInformation mi)
		{
			cs2D.Rigidbody2D.velocity = mi.MovementDirection * cs2D.BaseLocomotionAttribute.WalkSpeed * Time.fixedDeltaTime;
		}

		protected override void Run3D(ACharacterSystem3D cs3D, MotionInformation mi)
		{
			cs3D.transform.Rotate(0f, mi.MovementDirection.x * cs3D.BaseLocomotionAttribute.TurnSpeed * Time.fixedDeltaTime, 0f);
			cs3D.transform.Translate(0f, 0f, mi.MovementDirection.z * cs3D.BaseLocomotionAttribute.WalkSpeed* Time.fixedDeltaTime, Space.Self);
		}
	}
}
